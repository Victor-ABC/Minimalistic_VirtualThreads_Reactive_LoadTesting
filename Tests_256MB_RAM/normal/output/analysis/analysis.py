...
def read_csv(file_path):
    ...
    df_csv = pd.read_csv(file_path, header=None, names=['timestamp', 'freeRamPercentage', 'cpuUsagePercentage', 'totalMemory', 'maxMemory'])
...
def main():
    ...
    start_vus = end_vus - ((config_data['iterations'] - 1) * 10) #todo: 10 bzw. 100 sind die Schrittweite der VUS!!!

    # Create an empty DataFrame to store the results
    final_df = pd.DataFrame()

    for vus in range(start_vus, end_vus + 1, 10):

...
def read_and_visualize_csv(csv_file_path):
    ...
    # Plot maxHeap on the right axis
    color_max_heap = 'tab:orange'
    ax2.plot(df.index, df['avg_maxHeap'], color=color_max_heap, linestyle='dashed', marker='o', label='Heap (MB)')
    ax2.tick_params(axis='y', labelcolor=color_max_heap)

    # Plot freeHeapPercentage on the right axis
    color_free_heap_percentage = 'tab:purple'
    ax2.plot(df.index, df['avg_freeHeapPercentage'], color=color_free_heap_percentage, linestyle='dashed', marker='o', label='Free Heap (%)')
    ax2.tick_params(axis='y', labelcolor=color_free_heap_percentage)

    # Plot usedHeapPercentage on the right axis
    color_used_heap_percentage = 'tab:green'
    ax2.set_ylabel('Percentage (%)', color=color_used_heap_percentage)
    ax2.plot(df.index, df['avg_usedHeapPercentage'], color=color_used_heap_percentage, linestyle='dashed', marker='o', label='Used Heap (%)')
    ax2.tick_params(axis='y', labelcolor=color_used_heap_percentage)

    # Plot usedCPU on the right axis
    color_used_cpu = 'tab:blue'
    ax2.plot(df.index, df['avg_usedCPU'], color=color_used_cpu, linestyle='dashed', marker='o', label='Used CPU')
    ax2.tick_params(axis='y', labelcolor=color_used_cpu)
